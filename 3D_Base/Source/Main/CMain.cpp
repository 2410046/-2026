...
//	定数.
//=================================================
const TCHAR WND_TITLE[] = _T( "ディゾナンス" );
const TCHAR APP_NAME[]	= _T( "コンポーサー" );
...
CMain::CMain()
	...
	: m_hWnd	( nullptr )
{
	CDirectX9::GetInstance();
	CDirectX11::GetInstance();
}
...
HRESULT CMain::Create()
{
	...
	//アセット生成
	AssetManager::GetInstance()->Create(m_hWnd);
	// 最初のシーンを設定（タイトル画面）
	SceneManager::GetInstance()->NewScene(m_hWnd);
