...
HRESULT CSprite2D::CreateTexture( LPCTSTR lpFileName )
{
	...
	if( FAILED( D3DX11CreateShaderResourceViewFromFile(
		CDirectX11::GetDevice(),		//リソースを使用するデバイスのポインタ.
		lpFileName,			//ファイル名.
		nullptr, nullptr,
...
void CSprite2D::Render()
{
	//ワールド行列.
	D3DXMATRIX	mWorld;
	D3DXMATRIX	mTrans, mRot, mScale,mCenter;
	...
	float w = m_SpriteState.Disp.w;
	float h = m_SpriteState.Disp.h;
	D3DXMatrixTranslation(&mCenter, -w / 2.0f, -h / 2.0f, 0.0f);



	if (m_vRotation.x != 0.0f || m_vRotation.y != 0.0f || m_vRotation.z != 0.0f)
	{
		// 回転しているときだけ中心補正を入れる
	// 回転しているときだけ中心補正を入れる
		float w = m_SpriteState.Disp.w;
		...
		// スケールを設定
		//m_vScale = D3DXVECTOR3(1.5f, 1.5f, 1.5f); // 横・縦2倍
		...
		// 行列の掛ける順番: 中心補正 → スケール → 回転 → 平行移動
		mWorld = mCenter * mScale * mRot * mTrans;
	...
	if (SUCCEEDED(
		CDirectX11::GetContext()->Map( m_pConstantBuffer,
			0, D3D11_MAP_WRITE_DISCARD, 0, &pData )))
	{
		...
		CDirectX11::GetContext()->Unmap(m_pConstantBuffer, 0);
	}

	//このコンスタントバッファをどのシェーダで使うか？.
	CDirectX11::GetContext()->VSSetConstantBuffers( 0, 1, &m_pConstantBuffer );
	CDirectX11::GetContext()->PSSetConstantBuffers( 0, 1, &m_pConstantBuffer );

	//頂点バッファをセット.
	UINT stride = sizeof( VERTEX );	//データの間隔.
	UINT offset = 0;
	CDirectX11::GetContext()->IASetVertexBuffers( 0, 1,
	...
	CDirectX11::GetContext()->PSSetShaderResources( 0, 1, &m_pTexture );

	//アルファブレンド有効にする.
	CDirectX11::GetInstance()->SetAlphaBlend( true );

	//プリミティブをレンダリング.
	CDirectX11::GetContext()->Draw(4, 0);//板ポリ（頂点4つ分）.
