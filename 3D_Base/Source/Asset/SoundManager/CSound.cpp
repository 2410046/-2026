...
{
	static const std::wstring TEX_DIR = L"Data\\Sound\\";
	std::wstring fullPath = TEX_DIR + sFileName;
	//初期設定.
	...
	wsprintf( cmd, _T("open %s alias %s"), fullPath.c_str(), m_sAlias);
...
//  音声を途中から再生する関数.
//==============================================================================
bool CSound::PlayFrom(DWORD startMs, bool bNotify)
{
	// コマンド
	TCHAR cmd[STR_BUFF_MAX] = _T("");

	if (bNotify == true) {
		wsprintf(cmd, _T("play %s from %lu notify"), m_sAlias, startMs);
	}
	else {
		wsprintf(cmd, _T("play %s from %lu"), m_sAlias, startMs);
	}
...
DWORD CSound::GetSound()
{

		TCHAR buf[128] = _T("");
		TCHAR cmd[STR_BUFF_MAX] = _T("");

		wsprintf(cmd, _T("status %s position"), m_sAlias);
		if (mciSendString(cmd, buf, 128, nullptr) == 0) {
			return _ttoi(buf);
		}
		return 0;
