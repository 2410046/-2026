...
public:
	...
	void SetDiffuse(float r, float g, float b, float a)
	{
		...
		for (DWORD i = 0; i < m_NumAttr; ++i)
		{
			...
			if (matID < m_NumAttr)
				m_pMaterials[matID].Diffuse = m_Diffuse;
		}
	}

	// Ambient 色
	void SetAmbient(D3DXVECTOR4 rgba)
	{
		m_Ambient = rgba;
		for (DWORD i = 0; i < m_NumAttr; ++i)
		{
			DWORD matID = m_AttrID[i];
			if (matID < m_NumAttr)
				m_pMaterials[matID].Ambient = m_Ambient;
		}
	}
	...
	void SetSpecular(float r, float g, float b, float a)
	{
		...
		for (DWORD i = 0; i < m_NumAttr; ++i)
		{
			DWORD matID = m_AttrID[i];
			if (matID < m_NumAttr)
				m_pMaterials[matID].Specular = m_Specular;
		}
	}
	//クオータニオンの設定
	void SetQuaternion(D3DXQUATERNION Quat) { m_Quaternion = Quat; }	//クォータニオンの回転.
	//メッシュを取得.
...
private:
	...
	D3DXVECTOR4				m_Diffuse;	//ディフューズ色(拡散反射色).
	D3DXVECTOR4				m_Ambient;	//アンビエント色(環境色).
	D3DXVECTOR4				m_Specular;	//スペキュラ色(鏡面反射色).
