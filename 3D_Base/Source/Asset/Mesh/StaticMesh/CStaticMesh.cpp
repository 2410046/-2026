	...
	for( DWORD No = 0; No < m_Model.NumMaterials; No++ )
	{
			...
			&& strlen( d3dxMaterials[No].pTextureFilename ) > 0 )
		{
			...
			if( FAILED( D3DX11CreateShaderResourceViewFromFile(
				CDirectX11::GetDevice(), m_pMaterials[No].TextureName,//テクスチャファイル名.
				nullptr, nullptr,
...
HRESULT CStaticMesh::CreateSampler()
{
	...
	//サンプラ作成.
	if( FAILED(CDirectX11::GetDevice()->CreateSamplerState(
...
HRESULT CStaticMesh::CreateConstantBuffer()
{
	...
	//コンスタントバッファの作成.
	if( FAILED(
		CDirectX11::GetDevice()->CreateBuffer( &cb, nullptr, &m_pCBufferPerMesh ) ) )
	{
	...
	//コンスタントバッファの作成.
	if( FAILED(
		CDirectX11::GetDevice()->CreateBuffer( &cb, nullptr, &m_pCBufferPerMaterial ) ) )
	{
	...
	//コンスタントバッファの作成.
	if( FAILED(
		CDirectX11::GetDevice()->CreateBuffer( &cb, nullptr, &m_pCBufferPerFrame ) ) )
	{
	...
	LIGHT& Light, D3DXVECTOR3& CamPos)
{
	...
	D3DXMATRIX mWorld, mScale, mQRot, mTran;
	...
	//クォータニオンから回転行列を作成.
	D3DXMatrixRotationQuaternion(&mQRot, &m_Quaternion);

	//ワールド行列作成.
	//拡縮×回転×移動 ※順番がとても大切！！.
	mWorld = mScale * mQRot * mTran;


	//使用するシェーダのセット.
	CDirectX11::GetContext()->VSSetShader(m_pVertexShader, nullptr, 0);	//頂点シェーダ.
	CDirectX11::GetContext()->PSSetShader(m_pPixelShader, nullptr, 0);	//ピクセルシェーダ.

	//シェーダのコンスタントバッファに各種データを渡す.
	D3D11_MAPPED_SUBRESOURCE pData;
	//バッファ内のデータの書き換え開始時にMap.
	if (SUCCEEDED(CDirectX11::GetContext()->Map(
	...
	CDirectX11::GetContext()->VSSetConstantBuffers(	2, 1, &m_pCBufferPerFrame );	//頂点シェーダ.
	CDirectX11::GetContext()->PSSetConstantBuffers(	2, 1, &m_pCBufferPerFrame );	//ピクセルシェーダ.
	...
	D3DXMATRIX& mWorld, D3DXMATRIX& mView, D3DXMATRIX& mProj)
{
	...
	//このコンスタントバッファをどのシェーダで使用するか？.
	CDirectX11::GetContext()->VSSetConstantBuffers( 0, 1, &m_pCBufferPerMesh );	//頂点シェーダ.
	CDirectX11::GetContext()->PSSetConstantBuffers( 0, 1, &m_pCBufferPerMesh );	//ピクセルシェーダ.

	//頂点インプットレイアウトをセット.
	CDirectX11::GetContext()->IASetInputLayout( m_pVertexLayout );

	//プリミティブ・トポロジーをセット.
	CDirectX11::GetContext()->IASetPrimitiveTopology(
	...
	for( DWORD No = 0; No < m_NumAttr; No++ )
	{
		...
		if( SUCCEEDED(
			CDirectX11::GetContext()->Map(m_pCBufferPerMaterial,
		...
		CDirectX11::GetContext()->VSSetConstantBuffers( 1, 1, &m_pCBufferPerMaterial );
		CDirectX11::GetContext()->PSSetConstantBuffers( 1, 1, &m_pCBufferPerMaterial );

		//テクスチャをシェーダに渡す.
		if (m_pMaterials[m_AttrID[No]].pTexture != nullptr) {
			//テクスチャがあるとき.
			CDirectX11::GetContext()->PSSetSamplers( 0, 1, &m_pSampleLinear );
			CDirectX11::GetContext()->PSSetShaderResources(
				0, 1, &m_pMaterials[m_AttrID[No]].pTexture );
		}
		else {
			//テクスチャがないとき.
			ID3D11ShaderResourceView* pNothing[1] = { 0 };
			CDirectX11::GetContext()->PSSetShaderResources( 0, 1, pNothing );
		}

		//プリミティブ(ポリゴン)をレンダリング.
		CDirectX11::GetContext()->DrawIndexed(
