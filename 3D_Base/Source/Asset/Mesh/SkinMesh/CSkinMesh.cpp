...
HRESULT	CSkinMesh::CreateShader()
{
	...
	if( FAILED(
		CDirectX11::GetDevice()->CreateVertexShader(
	...
	//頂点インプットレイアウトを作成
	if( FAILED(
		CDirectX11::GetDevice()->CreateInputLayout(
	...
	if( FAILED(
		CDirectX11::GetDevice()->CreatePixelShader(
			pCompiledShader->GetBufferPointer(),
			pCompiledShader->GetBufferSize(),
...
HRESULT CSkinMesh::LoadXMesh( LPCTSTR FileName )
{
	...
	m_pD3dxMesh->LoadMeshFromX( CDirectX9::GetDevice(), FileName );
	...
	DWORD Size, int* pIndex, ID3D11Buffer** ppIndexBuffer )
{
	...
	if( FAILED(
		CDirectX11::GetDevice()->CreateBuffer( &bd, &InitData, ppIndexBuffer ) ) )
	{
		return E_FAIL;
	}
...
//	LPD3DXMESH pD3DXMesh = pFrame->pMeshContainer->MeshData.pMesh;//D3DXメッシュ(ここから・・・).
	...
	for( DWORD i = 0; i < pAppMesh->NumMaterial; i++ )
	{
		...
		if( pAppMesh->pMaterial[i].TextureName[0] != 0
			&& FAILED(
				...
				CDirectX11::GetDevice(), pAppMesh->pMaterial[i].TextureName,
	...
	HRESULT hRslt = S_OK;
	if( FAILED(
		CDirectX11::GetDevice()->CreateBuffer(
	...
	SKIN_PARTS_MESH* pMesh, D3DXMATRIX World, MYMESHCONTAINER* pContainer )
{
	...
	UINT offset = 0;
	CDirectX11::GetContext()->IASetVertexBuffers(
		0, 1, &pMesh->pVertexBuffer, &stride, &offset );

	//使用するシェーダのセット.
	CDirectX11::GetContext()->VSSetShader( m_pVertexShader, nullptr, 0 );
	CDirectX11::GetContext()->PSSetShader( m_pPixelShader, nullptr, 0 );

	//頂点インプットレイアウトをセット.
	CDirectX11::GetContext()->IASetInputLayout( m_pVertexLayout );

	//プリミティブ・トポロジーをセット.
	CDirectX11::GetContext()->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST );
	...
	for( DWORD i = 0; i < pMesh->NumMaterial; i++ )
	{
		...
		stride	= sizeof( int );
		offset	= 0;
		CDirectX11::GetContext()->IASetIndexBuffer(
		...
		//描画.
		CDirectX11::GetContext()->DrawIndexed( pMesh->pMaterial[i].NumFace * 3, 0, 0 );
	}
}
...
void CSkinMesh::SendCBufferPerBone(	SKIN_PARTS_MESH* pMesh )
{
			...
			D3D11_MAP_WRITE_DISCARD, 0, &pData ) ) )
	{
		...
		CDirectX11::GetContext()->Unmap( m_pCBufferPerBone, 0 );
	}
	CDirectX11::GetContext()->VSSetConstantBuffers( enCBSlot::Bones, 1, &m_pCBufferPerBone );
	CDirectX11::GetContext()->PSSetConstantBuffers( enCBSlot::Bones, 1, &m_pCBufferPerBone );
}
...
void CSkinMesh::SendCBufferPerFrame()
{
			...
			D3D11_MAP_WRITE_DISCARD, 0, &pData ) ) )
	{
		...
		CDirectX11::GetContext()->Unmap( m_pCBufferPerFrame, 0 );
	}
	CDirectX11::GetContext()->VSSetConstantBuffers( enCBSlot::Frame, 1, &m_pCBufferPerFrame );
	CDirectX11::GetContext()->PSSetConstantBuffers( enCBSlot::Frame, 1, &m_pCBufferPerFrame );
}
...
void CSkinMesh::SendCBufferPerMesh()
{
			...
			D3D11_MAP_WRITE_DISCARD, 0, &pDat )))
	{
		...
		memcpy_s( pDat.pData, pDat.RowPitch,
			reinterpret_cast<void*>( &cb ), sizeof( cb ));
		CDirectX11::GetContext()->Unmap( m_pCBufferPerMesh, 0 );
	}

	CDirectX11::GetContext()->VSSetConstantBuffers( enCBSlot::Mesh, 1, &m_pCBufferPerMesh );
	CDirectX11::GetContext()->PSSetConstantBuffers( enCBSlot::Mesh, 1, &m_pCBufferPerMesh );
}
...
void CSkinMesh::SendCBufferPerMaterial( MY_SKINMATERIAL* pMaterial )
{
			...
			D3D11_MAP_WRITE_DISCARD, 0, &pDat )))
	{
		...
		CDirectX11::GetContext()->Unmap( m_pCBufferPerMaterial, 0 );
	}

	CDirectX11::GetContext()->VSSetConstantBuffers( enCBSlot::Material, 1, &m_pCBufferPerMaterial );
	CDirectX11::GetContext()->PSSetConstantBuffers( enCBSlot::Material, 1, &m_pCBufferPerMaterial );
}
...
void CSkinMesh::SendTexture( MY_SKINMATERIAL* pMaterial )
{
	...
	{
		CDirectX11::GetContext()->PSSetSamplers( 0, 1, &m_pSampleLinear );
		CDirectX11::GetContext()->PSSetShaderResources( 0, 1, &pMaterial->pTexture );
	}
	else
	{
		ID3D11ShaderResourceView* Nothing[1] = { 0 };
		CDirectX11::GetContext()->PSSetShaderResources( 0, 1, Nothing );
	}
}
	...
	UINT Size)
{
	...
	if (FAILED(
		CDirectX11::GetDevice()->CreateBuffer( &cb, nullptr, pConstantBuffer )))
	{
		return E_FAIL;
	}
	return S_OK;
...
HRESULT CSkinMesh::CreateSampler( ID3D11SamplerState** pSampler )
{
	...
	if (FAILED(
		CDirectX11::GetDevice()->CreateSamplerState( &SamDesc, &m_pSampleLinear )))
	{
		return E_FAIL;
	}
	return S_OK;
